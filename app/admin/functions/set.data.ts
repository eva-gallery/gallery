...
export async function AdminSetData(
  admin: AdminType,
  formData: FormData,
...
export async function AdminSetDataJson(
  admin: AdminType,
  formData: FormData,
  method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE",
  endpoint: string
  ...
  if (admin.modul == "designer" && admin.action == "edit") {
    method = "PUT";
    endpoint = "/admin/designer/room/save";
  }
  if (admin.modul == "designer" && admin.action == "list") {
    method = "PUT";
    endpoint = "/admin/designer/room/save";
  }
  ...
  if (error) {
    ...
    if (admin.modul == "designer" && admin.action == "list") {
      redirect("/admin/designer/detail/" + json.id);
    }

    switch (admin.action) {
      case "login":
        const cookie = response?.headers?.["set-cookie"]?.find((cookie) =>
          cookie.startsWith("BEARER_TOKEN=")
        );
        if (cookie) {
